...
public:
  InputController(Root* root, OIS::Keyboard *keyboard, OIS::Mouse *mouse) : mRoot(root), mKeyboard(keyboard), mMouse(mouse)
  {
	...
	mProfessorInitPos = mProfessorNode->getPosition();
  ...
  bool frameStarted(const FrameEvent &evt)
  {
	...
	static Vector3 beforePos = mProfessorNode->getPosition();
	Vector3 offset = mProfessorNode->getPosition() - beforePos;
    mCamera->moveRelative(offset + mCameraMoveVector);
	beforePos = mProfessorNode->getPosition();
...
private:
  ...
  Entity * mProfessorEntity;
  Vector3 mProfessorInitPos;
};
...
public:
  ...
  void go(void)
  {
	...
	mCamera->setPosition(0.f, 100.f, 500.f);
	mCamera->lookAt(0.f, 100.f, 0.f);

	mViewport = mWindow->addViewport(mCamera, 0);
	mViewport->setBackgroundColour(ColourValue(0.f, 0.f, 0.5f));
	...
	mMapCamera->setPosition(0.f, 800.f, 1.f);
	mMapCamera->lookAt(0.f, 0.f, 0.f);
	
	mMapViewport = mWindow->addViewport(mMapCamera, 1, 0.79, 0.01, 0.2, 0.2);
	mMapViewport->setBackgroundColour(ColourValue(0.f, 0.f, 1.f));
